local library = {
	...
	colors = {
		main = Color3.fromRGB(255, 240, 25),
...
function library:CreateWindow(options, ...)
	...
	function windowFunctions:CreateDesigner(options, ...)
        ...
        local details2section = designer:CreateSection({
        ...
        local BackgroundThemes = {
            ["Heart Pattern"] = 6073763729,
            ["Abstract Pattern"] = 6073743880,
            ["Hexagon Pattern"] = 6073628849,
            ["Circle Pattern"] = 6071579805,
            ["Flower Pattern"] = 6071575946,
            ["Floral Pattern"] = 5553946671,
        }
		local flags = {}
		...
		local daaata = {{"AddTextbox", "__Designer.Textbox.ImageAssetID", backgroundsection, {
			Name = "Image Asset ID",
			Placeholder = "Insert Decal ID Here",
			Flag = "__Designer.Background.ImageAssetID",
			Value = "Insert Decal ID Here",
			...
			Callback = function(newValue)
                if newValue == "Nothing" then
                    innerBackdrop.Image = resolveid(0, "__Designer.Background.ImageAssetID")
                    designerelements["__Designer.Textbox.ImageAssetID"]:Set("Insert Decal ID Here")
                else
                    innerBackdrop.Image = resolveid(BackgroundThemes[newValue], library_flags["__Designer.Background.ImageAssetID"])
                    designerelements["__Designer.Textbox.ImageAssetID"]:RawSet("rbxassetid://" .. BackgroundThemes[newValue])
                end
            end
		...
		if options.Credits then
			local typ = type(options.Credits)
			if typ == "string" then
        ...
        if options.DiscordServer then
			local typ = type(options.DiscordServer)
			if typ == "string" then
				daaata[1 + #daaata] = {"AddLabel", "__Designer.Label.Creator", details2section, {
					Text = options.DiscordServer
				}}
			end
        end
